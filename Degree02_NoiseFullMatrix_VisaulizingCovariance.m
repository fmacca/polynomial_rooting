...
folder_name='Results/Degree02_NoiseFullMatrix_VisualizingCovariance'; 
...
for ii=1:(2*N)
    for jj=ii:(2*N)
        if(~(ii==1 && jj==3) && ~(ii==2 && jj==4)) || ((ii==1 && jj==3) || (ii==2 && jj==4))    
            subplot(2*N,2*N,(ii-1)*2*N+jj)
            plot(log(abs(Projection)),abs(reshape(MSE_analytic_tilda(ii,jj,:),SNR_nsteps,1)),'-');hold on;
            plot(log(abs(Projection)),abs(reshape(Covariance_simulated_tilda(ii,jj,:),SNR_nsteps,1)),'xr');
            plot(log(abs(Projection)),abs(reshape(MSE_simulated_tilda(ii,jj,:),SNR_nsteps,1)),'xk');
            ylim([10^(-4) 10^2]);
            if(ii==1 & jj==1)
                legend([ strcat("MSE analytic"); strcat("Cov simulated"); strcat("MSE simulated")],'Location','southwest');
            end
