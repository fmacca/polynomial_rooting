%% Bias vs Sigma Figure 1 Special case
clear all
close all
clc
...
folder_name='Results/Degree02_NoiseCircular_MSEVsSigma'; %Name for the results folder: it should be named after the kind of test performed
...
sigma_scale = logspace(-3,0,40); %[0.01:0.02:0.8]; %[0.01:0.04:1.6];%[0.01:0.01:0.4]; % standard deviation of the noise on coefficients
sigma_nsteps=numel(sigma_scale); 
K=10^4;%10^5; % Number of iterations per simulation (n of noisy measurements per polynomial)
Repeat_experiment = 20; % 10^2
...
Sigma = eye(2*N); % In case we want to randomize: generate_covariance(N,1,'circular');
...
loglog(sigma_scale, reshape(toplot,sigma_nsteps,1)/K, 'b-');hold on;
...
axis([sigma_scale(1) sigma_scale(end) 10^-10 10^-1])

title("MSE of the estimator vs \sigma")

legend(["Theoretical upper bound bias^2", "Analytical MSE prediction", "MSE from simualation"],'Location','northwest')
