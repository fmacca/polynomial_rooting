...
N=4;
% folder_name=strcat('Results/Degree0',int2str(N),'_NoiseCircular_TestNormality');
folder_name=strcat('Results/Degree0',int2str(N),'_NoiseFullMatrix_TestNormality');
...
plot(x,pihat(:,2),'r.');
xline(sep_line,'b--');
legend("P-value of Henzle-Zirlker normality test","Level $\alpha=0.05$ for H-Z test","Fitted model","Separating value $\overline{\gamma}_{HZ}$","Location","Northwest","interpreter","latex");
title("Henzle-Zirlker normality test");
...
yline(lev,'b--');
xline(0.05,'r');
xlabel("P-value of Henzle-Zirlker normality test");
ylabel("Probability from fitted model");
legend("Fitted vs actual","Chosen model threshold","Level $\alpha=0.05$ for H-Z test","Location","Southeast","interpreter","latex")
title("Logistic regression fitted probabilities vs H-Z test P-values");
hold off
